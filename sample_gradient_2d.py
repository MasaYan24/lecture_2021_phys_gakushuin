...
def create_model(
    ...
    logger = getLogger(__name__).getChild("create_model")
    if bias is None:
...
def main() -> None:
    ...
    model_a = create_model(x_tensor, y_tensor, loss_list, a_list, b_list=None, weight=a_list[0], bias=None, lr=0.01)
    ...
    xt = np.linspace(0.0, 1.3, num=10).reshape((10, 1))
    ...
    model_ab = create_model(x_tensor, y_tensor, loss_list, a_list, b_list, weight=a_list[0], bias=b_list[0])
    logger.info(
